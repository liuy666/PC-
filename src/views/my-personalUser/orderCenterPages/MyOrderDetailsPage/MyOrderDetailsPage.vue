    ...
    <div class="MyOrderDetailsPage">
        ...
        <div class="booking-details">
            ...
            <div class="effective-date">
                有效日期：<span class="effective-date-text">{{ylks}}日内有效</span>
            </div>
...
export default {
      ...
      colnames: [
...

        {
          ...
          render: (h, params) => {
            ...
            } else if(params.row.pmva === '全退订(退款中)' || params.row.pmva === '退款中' || params.row.pmva === '已支付(退款中)' || params.row.pmva ===  '全退订(退订已退款)' || params.row.pmva ===  '全退订'){
                return h("div", [
                    h(
                        "span",
                        {
            ...
            } else if(params.row.pmva === '垃圾（作废）订单'){
                return h("div", [
                    h(
                        "span",
                        {
            ...
            if (params.row.ddzt == "02") {
              return h("div", [
                h("a",{style: {display: "block",marginTop:'5px'},
                    on: {
                      click: () => {
                        if(params.row.ddzt === '11'){
                            this.$Modal.error({
                              title: "温馨提示",
                              content: '订单已出票无法修改',
                              onOk: () => {
                                setTimeout(() => {
                                  this.$router.go({name: 'MyOrderDetailsPage'})
                                },500);
                              }
                            });
                          return
                ...
                h("a",{style: {display: "block",marginTop:'5px'},
                    on: {
                      click: async () => {
                        ...
                        //   return
                        // } else {
                        //   if(this.systemLength !== 0) {
                        //     this.$Modal.error({
                        //       title: "温馨提示",
                        //       content: '实名制门票仅支持全部退订'
                        //     });
                        //     return
                        //   }
                        ...
                        await this.RevisYDDH();
                          if(this.codeNumber === 400) {
                            this.ReviseReservationOrder = false;
                          } else {
                            this.ReviseReservationOrder = true;
                          }
                        // if(this.codeNumber === 200) {
                        //   this.ReviseReservationOrder = false;
                        // } else {
                        //   this.ReviseReservationOrder = true;
                        // }
  ...
  methods: {
    ITviewDetailsAxios() {
      ...
      this.$http
        ...
        .then(response => {
          ...
          this.systemLength = response.data.data.realnames.length;
          ...
          this.ylks = yorderDataList[0].dtstartdate + ' 至 ' + yorderDataList[0].dtenddate + ' '

          if (this.pmvaName == "已支付" || this.pmvaName == "已出票") {
            this.numberPmva = morderList.bankdata + ' ' + morderList.banktime;
            this.completeNumber = 2;
            this.offStocks = false;
          } else if (this.pmvaName == "退款中" || this.pmvaName == "退款失败") {
            this.offStocks = false;
            this.refund = true;
          } else if(this.pmvaName !== "未付款") {
            this.offStocks = false;
          } else if(this.pmvaName === "未付款") {
            this.offStocks = true;
          ...
          torderListData.forEach(item => {
            item["pmva"] = morderList.pmva;
            item["ddzt"] = morderList.ddzt;
            item['length'] = realnamesList.length;
            item['endtime'] = yorderDataList[0].endtime;
            item['starttime'] = yorderDataList[0].starttime;
          });
          ...
          let time = this.GetDateDiff(morderList.orda)
          this.pastDueDate = moment(new Date(`${time} ${morderList.orti}`).getTime() + 30 * 60 * 1000).format('YYYY-MM-DD HH:mm');
    ...
    changeDateVisitOk() {
      ...
      let oridName = this.orderConfirmationNumber;
    ...
    TicketTakerInformationOk() {
      this.isShowLoading = true;
      let reg1 = /^(13[0-9]|14[5-9]|15[0-9]|166|17[0-8]|18[0-9]|19[8-9])[0-9]{8}$/;
      let reg2 = /(^[1-9]\d{5}(18|19|([23]\d))\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}[0-9Xx]$)|(^[1-9]\d{5}\d{2}((0[1-9])|(10|11|12))(([0-2][1-9])|10|20|30|31)\d{3}$)/;
      if(reg1.test(this.formDatas.val3) === false) {
          this.$Modal.error({
            title: "温馨提示",
            content: '手机号输入有误！'
          });
          this.isShowLoading = false;
          return;
      } else if(reg2.test(this.formDatas.val2) === false) {
          this.$Modal.error({
            title: "温馨提示",
            content: '输入有误！'
          });
      ...
      this.$http.get(this.baseURL + "/ec/orderManage/v1/orderRePay", {
        ...
        .then(response => {
          if (response.data.code === 400) {
            ...
            this.skiprl = response.data.data.url + skipURL;
          ...
          if(response.data.code === 200){
              this.$Modal.success({
                ...
                onOk: () => {
                    setTimeout(() => {
                        this.$router.push({
                          query: {orid:this.dataListMorder.orid, usid:sessionStorage.getItem("userName")},
