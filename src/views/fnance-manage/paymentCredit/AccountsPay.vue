...
<div class="purchaseOrder">
    ...
    <div class="w_query" :style="{fontSize:'14px'}">
      <Row :style="styleB">
        ...
        <Col span="8">
            <span>结算状态：</span>
            <Select v-model="form.settleStatus" :style="style">
        ...
        <Row :style="styleB">
          ...
          <Row :style="styleB">
             <Col span="12">
                <span>供应商名称：</span>
                <Input v-model="form.agent" placeholder="请输入供应商名称" :style="style" />
            </Col>
          </Row>
          <Row >
            <Col span="24" >
                <Button class="btnBg" shape="circle" :style="[styleW,styleR]" id="myBtn" @click="query">查询</Button>
...
export default {
  ...
  data () {
    return {
      ...
      id: {
        ...
        consumerId: '',
        gatherId: '',//收款详情ID
        isSurePay: false,//是否已经付款
        isAccountsPay: false,//判断时候是应付款进入
      },
      gatherBtn: false,
      form: {
        oddNum: '',//单号
        ...
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              ...
              }, this.columnsData[params.index]._settleStatus === 'FINISHED' ? '' : ' 付款信息提交')
            ]);
          }
        }
      ],
  ...
  async mounted () {
    let idArr = await getId.getId(this.fm, this.token, 2);
    if (idArr) {
